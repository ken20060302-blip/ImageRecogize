...
namespace DIP
{
    public partial class DIPSample : Form
    {
        ...
        private void 直ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            // 抓取目前主畫面中間正在顯示的圖片視窗
            MSForm currentForm = ActiveMdiChild as MSForm;

            // 防呆：如果沒有開啟任何圖片，就跳出提示並結束動作
        ...
        private void 等話ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            // 1. 抓取目前主畫面中間正在顯示的圖片視窗
            MSForm currentForm = ActiveMdiChild as MSForm;
            if (currentForm == null || currentForm.pBitmap == null)
            {
                MessageBox.Show("請先開啟一張圖片！");
            ...
            // ★ (新增) 呼叫 Form5，顯示等化後的「直方圖」！
            Form5 eqHistogramForm = new Form5();
            eqHistogramForm.Text = "等化後直方圖"; // 幫視窗改個名字方便辨識
            eqHistogramForm.TargetImage = NpBitmap; // 把剛剛算完的等化圖片丟給 Form5 去畫圖
